...
void printDistributions(Tdistrib distrib, Pdistribdata result, int don, int printrawdata, int indexNo)
{
	...
	for(i=0;i<result->size;i++)
	{
		...
		if (don == 1)
			packet_size[indexNo][i] = (double) result->data[i];
		else
			packet_time[indexNo][i] = (double) result->data[i];
	}

	fprintf(traffic_stat,"Internal stats of the %s generation: %d values, mean=%g\n",
	...
	for(i=0;i<=PRINT_DISTRIB_COUNT_NB;i++)
	{
		...
		fprintf(traffic_stat, "%s;" PercentMyInt ";%d\n", DonOffStr(don), Xval, count[i]);
	}
}
